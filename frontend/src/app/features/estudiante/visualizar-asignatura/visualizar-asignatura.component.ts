...
import { Entrega, Tarea } from '../../../shared/models';
...
interface TareaConEstado extends Tarea {
  entregada: boolean;
  vencida: boolean;
  nota: number | null;
...
@Component({
  ...
  standalone: true,
  imports: [HeaderComponent, FechaPipe],
  template: `
    <app-header />
    <div class="page-bg">
                  ...
                  <tbody class="divide-y divide-gray-100">
                    @for (t of tareasSorted(); track t.tarea_id) {
                      <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{{ t.fecha_inicio | fecha }}</td>
                        <td class="text-xs text-gray-500 whitespace-nowrap">{{ t.fecha_final | fecha }}</td>
                        <td>
                          @if (t.entregada && t.nota != null) {
                            <span class="nota-badge" [class]="claseNota(t.nota)">
                              {{ formatearNota(t.nota) }} · {{ etiquetaNota(t.nota) }}
                            </span>
                            @if (t.comentario) {
                              <p class="calificacion-comentario-vista">{{ t.comentario }}</p>
                            }
                          } @else if (t.entregada) {
                            <span class="nota-badge nota-badge--pendiente">Pendiente de calificar</span>
                          } @else {
                          ...
                          @if (t.vencida) {
                            @if (t.entregada) {
                              <span class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-green-100 text-green-700 border border-green-200 cursor-not-allowed">
                                {{ t.calificada ? 'Entregado' : 'Entregado (sin calificar)' }}
                              </span>
                            } @else {
                            ...
                            @if (t.entregada && t.calificada) {
                              <span class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-green-100 text-green-700 border border-green-200 cursor-not-allowed">
                                Entregado
                              </span>
                            } @else if (t.entregada) {
                              <button type="button" (click)="entregar(t)"
                              ...
                              <button type="button" (click)="entregar(t)" class="btn-primary text-xs py-1.5 px-3">
                                Entregar
                              </button>
                            }
                          }
                      ...
                      <tr><td colspan="6" class="text-center text-gray-400 py-6">Sin tareas asignadas</td></tr>
                    }
                  </tbody>
                </table>
              </div>
...
export class VisualizarAsignaturaComponent implements OnInit {
    ...
    this.api.post<TareasResponse>('/listarTareas', { group_id: this.groupId }).pipe(
      switchMap(({ tareas }) => {
        ...
        const checks = tareas.map(t =>
          this.api.post<IsEntregadaResponse>('/isEntregada', { tarea_id: t.tarea_id, user_id: userId }).pipe(
            catchError(() => of<IsEntregadaResponse>({ success: false, entrega: null }))
          )
        );
      ...
      next: ({ tareas, results }) => {
        const conEstado: TareaConEstado[] = tareas.map((t, i) => {
          ...
          const calificada = res?.calificada ?? entrega?.nota != null;
          return {
            ...t,
            entregada: res?.success ?? false,
  ...
  entregar(t: TareaConEstado): void {
    if (t.calificada) {
      sw.warning('Entrega bloqueada', 'Ya recibiste una calificación; no puedes modificar esta entrega.');
      return;
    }
