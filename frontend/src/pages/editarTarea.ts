...
document.getElementById('tareaForm')!.addEventListener('submit', async (e) => {
  ...
  try {
    const data = await api.post<ApiResponse>('/editarTarea', {
      tarea_id: session.getTareaId(), titulo, descripcion, fecha_inicio, fecha_final,
    });
    if (data.error) { await showError('Error', data.error); return; }
    await showSuccess('Tarea actualizada', data.message);
    history.back();
  } catch {
